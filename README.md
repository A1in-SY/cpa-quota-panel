...
- 厂商分类筛选（展示过滤，不做统计）：默认内置 `OpenCode Go / deepseek / minimax / 智谱 CodingPlan / Cline Pass`。
...
- 额度展示：`percent-windows`（OpenCode Go 三窗口进度）、`balance`（deepseek 余额）、`coding-plan`（minimax coding plan 各 model 的当前周期/本周剩余）、`zhipu-plan`（智谱 GLM Coding Plan 的 5 小时/每周/MCP 每月额度，走 `open.bigmodel.cn/api/monitor/usage/quota/limit`，普通 API key 即可）、`cline-plan`（Cline Pass 订阅的 5 小时/每周/每月三条进度，走 `api.cline.bot/api/v1/users/me/plan/usage-limits`，普通 Cline API key 即可）。
...
- Cline Pass 的 key 需以 `openai-compatibility`（base-url `https://api.cline.bot/api/v1`）之类的条目写进宿主 config 才在扫描范围内；该额度接口是订阅态接口，未订阅时返回 `data:null`，卡片显示「暂无额度数据」而非报错。
